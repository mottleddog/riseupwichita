// Demo UI logic for job/profile gating and demo data
(() => {
  ...
  const demoProfiles = [
    {id:'p1',first:'Ava',last:'Morrison',email:'ava.morrison@example.com',phone:'(316) 555-0101',primary:'Graphic Artist',specialties:['Logo Design','Print Layout'],bio:'Creative designer focused on clear branding.',resumeAttached:true,demo:true},
    {id:'p2',first:'Liam',last:'Parker',email:'liam.parker@example.com',phone:'(316) 555-0102',primary:'Landscaping',specialties:['Mowing','Mulching'],bio:'Detail-oriented outdoor worker.',resumeAttached:true,demo:true},
    {id:'p3',first:'Maya',last:'Singh',email:'maya.singh@example.com',phone:'(316) 555-0103',primary:'Caregiving',specialties:['Child Care','Elder Care'],bio:'Reliable and compassionate caregiver.',resumeAttached:false,demo:true},
    {id:'p4',first:'Noah',last:'Garcia',email:'noah.garcia@example.com',phone:'(316) 555-0104',primary:'Delivery',specialties:['Local Routes','Heavy Lifting'],bio:'Punctual driver with clean record.',resumeAttached:false,demo:true},
    {id:'p5',first:'Olivia',last:'Reed',email:'olivia.reed@example.com',phone:'(316) 555-0105',primary:'Food Service',specialties:['Line Cook','Front of House'],bio:'Customer-focused server and cook.',resumeAttached:true,demo:true},
    {id:'p6',first:'Ethan',last:'Brooks',email:'ethan.brooks@example.com',phone:'(316) 555-0106',primary:'Handyman',specialties:['Minor Repairs','Assembly'],bio:'Practical problem solver.',resumeAttached:true,demo:true},
    {id:'p7',first:'Sophia',last:'Lopez',email:'sophia.lopez@example.com',phone:'(316) 555-0107',primary:'Cleaning',specialties:['Deep Clean','Sanitization'],bio:'Thorough and efficient.',resumeAttached:false,demo:true},
    {id:'p8',first:'Mason',last:'Cruz',email:'mason.cruz@example.com',phone:'(316) 555-0108',primary:'Construction',specialties:['Demolition','Carpentry'],bio:'Skilled with tools and teamwork.',resumeAttached:true,demo:true},
    {id:'p9',first:'Isabella',last:'Nguyen',email:'isabella.nguyen@example.com',phone:'(316) 555-0109',primary:'Graphic Artist',specialties:['Digital Illustration','Social Media'],bio:'Versatile artist for digital campaigns.',resumeAttached:false,demo:true},
    {id:'p10',first:'Logan',last:'Kim',email:'logan.kim@example.com',phone:'(316) 555-01010',primary:'Delivery',specialties:['Courier','Customer Service'],bio:'Friendly and efficient courier.',resumeAttached:true,demo:true},
    {id:'p11',first:'Emma',last:'Dawson',email:'emma.dawson@example.com',phone:'(316) 555-0111',primary:'Food Service',specialties:['Prep Cook','Cash Handling'],bio:'Fast learner in busy kitchens.',resumeAttached:false,demo:true},
    {id:'p12',first:'Lucas',last:'Reyes',email:'lucas.reyes@example.com',phone:'(316) 555-0112',primary:'Cleaning',specialties:['Office Clean','Event Clean'],bio:'Reliable cleaner for recurring gigs.',resumeAttached:true,demo:true},
    {id:'p13',first:'Amelia',last:'Price',email:'amelia.price@example.com',phone:'(316) 555-0113',primary:'Caregiving',specialties:['Respite Care','Medication Reminders'],bio:'Patient and compassionate.',resumeAttached:true,demo:true},
    {id:'p14',first:'James',last:'Ford',email:'james.ford@example.com',phone:'(316) 555-0114',primary:'Handyman',specialties:['Plumbing Basics','Furniture Assembly'],bio:'Efficient problem solver.',resumeAttached:false,demo:true},
    {id:'p15',first:'Zoe',last:'Hart',email:'zoe.hart@example.com',phone:'(316) 555-0115',primary:'Graphic Artist',specialties:['Poster Design','Branding'],bio:'Bold visual communicator.',resumeAttached:true,demo:true}
  ];
  ...
  const demoJobs = [
    ...
    {id:'j12',title:'Grocery Delivery',company:'NeighborCart',location:'Wichita, KS',category:'Delivery',salary:'$14/hr',posted:'Today',description:'Pick-up and deliver grocery orders to local customers.',contact:{name:'Dispatch',email:'dispatch@neighborcart.example',phone:'(316) 444-1012'}},
  ...
  function renderProfiles(){
    ...
    demoProfiles.forEach(p=>{
      ...
      const av = el('div','profile-avatar', (p.first[0]+p.last[0]).toUpperCase());
  ...
  function openJob(job){
    ...
    const contactInner = el('div', isLoggedIn() ? '' : 'contact-blur small', isLoggedIn() ? `${job.contact.name} • ${job.contact.email} • ${job.contact.phone}` : `${job.contact.name} • ${job.contact.email} • ${job.contact.phone}`);
  ...
  function openResumeBuilder(){
    ...
    widget.innerHTML = '';
    const profile = getProfile();
    const h = el('h4',null,'Resume Builder (simple demo)');
    const txt = el('textarea',null); txt.style.width='100%'; txt.style.height='200px'; txt.value = `Name: ${profile.first} ${profile.last}\nEmail: ${profile.email}\nPrimary: ${profile.primary}\nSummary: ${profile.bio || ''}\nSkills: ${profile.specialties.join(', ')}`;
    const btn = el('button','btn primary','Attach Resume');
    btn.onclick = ()=>{ profile.resumeAttached = true; localStorage.setItem('ruf_profile',JSON.stringify(profile)); alert('Resume attached (demo). You can now apply.'); renderProfiles(); };
    const printBtn = el('button','btn','Print Resume'); printBtn.onclick = ()=>{ const w=window.open('about:blank'); w.document.write('<pre>'+txt.value.replace(/</g,'&lt;')+'</pre>'); w.print(); };
    widget.appendChild(h); widget.appendChild(txt); widget.appendChild(btn); widget.appendChild(printBtn);
  }
  ...
  function saveProfileFromForm(e){
    ...
    const data = {
      ...
      primary: f.primary.value, specialties: (f.specialties.value||'').split(',').map(s=>s.trim()).filter(Boolean).slice(0,2), bio:'', resumeAttached:false
    };
